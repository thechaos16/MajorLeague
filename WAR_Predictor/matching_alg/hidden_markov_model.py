...
class HiddenMarkovModel:
    def __init__(self,begin_prob,trans_matrix,means,covariance,number_of_samples=100):
...
if __name__=='__main__':
    ...
    trans_matrix = np.array([[0.7, 0.2, 0.1], [0.3, 0.5, 0.2], [0.3, 0.3, 0.4]])
    means = np.array([[0.0, 0.0], [3.0, -3.0], [5.0, 10.0]])
    covariance = np.tile(np.identity(2), (3, 1, 1))
    hmm_instance = HiddenMarkovModel(begin_prob,trans_matrix,means,covariance)
    X = []
    for i in range(100):
