...
def swp(data):
    # results (all-time swing percentage, swing percentage after 2 strikes)
    swing = [0,0]
    swa2 = [0,0]
                ...
                if data[i][1][j-1][0]>='A' and data[i][1][j-1][0]<='z':
                    ...
                    while 1:
                        if 'ejected' in data[i][1][j][check] or 'hit for' in data[i][1][j][check] or ' in ' in data[i][1][j][check]:
                            ...
                            break
                    if brake!=1:
                        data[i][1][j][check] = data[i][1][j][check].strip(',')
                        data[i][1][j][check] = data[i][1][j][check].strip(' ')
                        ...
                        if 'Ball'==data[i][1][j][check][0:4] or 'Strike'==data[i][1][j][check][0:6]:
                            ...
                            for k in range(len(temp)):
                                ...
                                if 'swinging' in temp[k]:
                                    ...
                                    # strike called
                                    sa2+=1
                                elif 'foul' in temp[k] or 'Foul' in temp[k]:
                                    if sa2==2:
                                        swa2[0]+=1
                                        swa2[1]+=1
                                    else:
                                        ...
                                        swing[1]+=1
                                        sa2+=1
                                elif 'looking' in temp[k]:
                                    sa2+=1
                            if len(data[i][1][j])>=check+2:
                                pitch[0]+=1
                                if 'walk' in data[i][1][j][check+1] or 'looking' in data[i][1][j][check+1] or 'hit by' in data[i][1][j][check+1]:
                                    continue
                                else:
                                    if sa2==3:
                                        continue
                                        ...
                                        pit2[0]+=1
                                        swa2[0]+=1
                                        if 'swinging' not in data[i][1][j][check+1]:
                                           swa2[1]+=1
                                    else:
                                        swing[0]+=1
                                        swing[1]+=1
...
f.write(str(res[1][0])+'\t'+str(res[1][1])+'\n')
f.write(str(res[2][0])+'\t'+str(res[2][1])+'\n')
f.write(str(res[3][0])+'\t'+str(res[3][1])+'\n')
f.close()
